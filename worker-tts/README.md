...
- **Mặc định async** (`_call_revid_tts_async`): `POST /api/v1/tts/async` lấy `task_id` → poll `GET /api/v1/tasks/{task_id}` mỗi `REVID_ASYNC_POLL` (5s) tới `status=completed`. Bền hơn với chương dài / API chậm. Config `REVID_ASYNC_MAX_WAIT` (600s) + `REVID_ASYNC_POLL` (5s).
...
- Mặc định **6 replicas** (ghi cố định trong `docker-compose.yml`).
- Override runtime: `docker compose --profile worker-tts up -d --scale worker-tts=N worker-tts`.
- Hạ replica nếu Revid API trả `429` / hết credit.
