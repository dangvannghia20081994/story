...
#
# Tuỳ chọn:
#   --with-crawler  Python crawler (cần worker-crawler/.env + worker-crawler/.venv)
...
# Chuẩn bị crawler: worker-crawler/README.md
...
for arg in "$@"; do
  if [[ "$arg" == "--with-crawler" ]]; then
    ...
    echo "  --with-crawler   Crawler Python worker.py (cần worker-crawler/.env + worker-crawler/.venv)."
...
if [[ "$WITH_CRAWLER" == "1" && "${SKIP_CRAWLER_WORKER:-}" != "1" && -f "$ROOT_DIR/worker-crawler/.env" ]]; then
  CRAWLER_PY=""
  if [[ -f "$ROOT_DIR/worker-crawler/.venv/Scripts/python.exe" ]]; then
    CRAWLER_PY="$ROOT_DIR/worker-crawler/.venv/Scripts/python.exe"
  elif [[ -f "$ROOT_DIR/worker-crawler/.venv/bin/python" ]]; then
    CRAWLER_PY="$ROOT_DIR/worker-crawler/.venv/bin/python"
  fi
  if [[ -n "$CRAWLER_PY" ]]; then
    start_service "crawler-worker" "$ROOT_DIR/worker-crawler" "$CRAWLER_PY worker.py"
  else
    echo "Warning: --with-crawler nhưng không thấy worker-crawler/.venv — bỏ qua crawler-worker. Tạo venv: cd worker-crawler && python -m venv .venv && … xem worker-crawler/README.md."
  fi
elif [[ "$WITH_CRAWLER" == "1" && "${SKIP_CRAWLER_WORKER:-}" != "1" ]]; then
  echo "Warning: --with-crawler nhưng thiếu worker-crawler/.env — bỏ qua crawler-worker (sao chép worker-crawler/.env.example → worker-crawler/.env)."
fi
