...
| `worker-tts.Dockerfile` | Image dùng chung cho 3 service `worker-tts` / `worker-tts-2` / `worker-tts-3` (profile `worker-tts`) — `context: ./worker-tts`, Revid TTS API + ffmpeg. Mỗi service **6 replicas** (`deploy.replicas: 6`) → tổng **18 luồng**, competing consumers cùng BLPOP `story:tts:queue`, mỗi nhóm dùng 1 API key Revid riêng |
...
| Worker-TTS (profile **`worker-tts`**) | `worker-tts/.env` + `.env.key2` + `.env.key3` (3 API key Revid, mỗi key 6 replicas → 18 luồng song song); override `--scale worker-tts=N --scale worker-tts-2=N --scale worker-tts-3=N` |
