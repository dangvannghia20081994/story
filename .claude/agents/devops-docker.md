...
| `crawler` | `crawler` | — | Playwright/Chromium; cần `crawler/.env` |
...
- **Không** đổi `backend/.env`, `frontend/.env.local`, `app/.env`, `crawler/.env`, `worker-tts/.env` — đó là env runtime (layer agent quản).
...
- **`crawler` + `worker-tts` profile**: mặc định KHÔNG chạy (`profiles:` chặn). Tốn RAM Chromium / model TTS.
...
- README folder app tương ứng (`backend/`, `frontend/`, `app/`, `crawler/`, `worker-tts/`) nếu compose env nhìn ra cho service đó
